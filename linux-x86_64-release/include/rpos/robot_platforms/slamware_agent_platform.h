...
namespace rpos { namespace robot_platforms {
    ...
    public:
        ...
        bool getOptAreas(features::location_provider::RectangleAreaMap &);
        bool addOptArea(const features::location_provider::RectangleAreaDescriptor & descriptor);
        bool addOptAreas(const features::location_provider::RectangleAreaMap & map);
        bool removeOptAreaById(const std::vector<std::uint32_t> & id);
        bool clearOptAreas(features::location_provider::MapKind mapKind = features::location_provider::MAINTENANCEAREAMAP);
